...
import com.wy.wydemo.model.enums.CommentTypeEnum;
...
public class CommentServiceImpl extends ServiceImpl<CommentMapper, Comment> implements CommentService {
    ...
    public void addComment(CommentReq comment) {
        ...
        Comment newComment = Comment.builder()
                .fromUid(StpUtil.getLoginIdAsLong())
                .toUid(comment.getToUid() != null ? Long.valueOf(comment.getToUid()) : null)
    ...
    private void notice(Comment comment, String fromNickname) {
        ...
        String title = "友链";
        // 回复用户id
        Long toUid = Long.valueOf(CommonConstant.BLOGGER_ID);
        // 父评论
        if (Objects.isNull(comment.getParentId())) {
            ...
            if (comment.getCommentType().equals(CommentTypeEnum.TALK.getType())) {
                title = "说说";
                toUid = Long.valueOf(talkMapper.selectOne(new LambdaQueryWrapper<Talk>()
    ...
    private void verifyComment(CommentReq comment) {
        if (comment.getCommentType().equals(CommentTypeEnum.ARTICLE.getType())) {
            Article article = articleMapper.selectOne(new LambdaQueryWrapper<Article>().select(Article::getId).eq(Article::getArticleType, comment.getTypeId()));
    ...
    private void sendEmail(Comment comment, User toUser, String title, String fromNickname) {
        ...
        if (comment.getIsCheck().equals(CommonConstant.TRUE)) {
            ...
            if (Objects.isNull(comment.getParentId())) {
                ...
                String createTime = DateUtil.formatLocalDateTime(comment.getCreateTime().toInstant().atZone(ZoneId.systemDefault()).toLocalDateTime());
