package com.wy.wydemo.service.impl;

import com.wy.wydemo.exception.ServiceException;
import com.wy.wydemo.strategy.UploadStrategy;
import com.wy.wydemo.utils.FileUtils;
import lombok.extern.slf4j.Slf4j;
import org.springframework.stereotype.Service;
import org.springframework.web.multipart.MultipartFile;

import java.io.IOException;
import java.io.InputStream;

/**
 * @description: 抽象上传模板
 * @class: AbstractUploadStrategyImpl
 * @author: yu_wei
 * @create: 2024/11/05 23:58
 */
@Slf4j
@Service
public abstract class AbstractUploadStrategyImpl implements UploadStrategy {
    
    @Override
    public String uploadFile(MultipartFile file, String path) {
        try {
            // 获取文件md5值
            String md5 = FileUtils.getMd5(file.getInputStream());
            // 获取文件扩展名
            String extName = FileUtils.getExtension(file);
            // 重新生成文件名
            String fileName = md5 + "." + extName;
            // 确保 path 和 fileName 之间有 "/"
            String fullPath = path.endsWith("/") ? path + fileName : path + "/" + fileName;
            
            // 判断文件是否已存在
            if (!exists(fullPath)) {
                // 不存在则继续上传
                upload(path.endsWith("/") ? path : path + "/", fileName, file.getInputStream());
            }
            
            // 返回文件访问路径
            return getFileAccessUrl(fullPath);
        } catch (Exception e) {
            log.info("uploadFile fail, error is {}", e.getMessage());
            throw new ServiceException("文件上传失败");
        }
    }

    /**
     * 判断文件是否存在
     *
     * @param filePath 文件路径
     * @return {@link Boolean}
     */
    public abstract Boolean exists(String filePath);
    
    /**
     * 上传
     *
     * @param path        路径
     * @param fileName    文件名
     * @param inputStream 输入流
     * @throws IOException io异常
     */
    public abstract void upload(String path, String fileName, InputStream inputStream) throws IOException;
    
    /**
     * 得到文件访问url
     *
     * @param filePath 文件路径
     * @return {@link String} 文件url
     */
    public abstract String getFileAccessUrl(String filePath);
}
